...
class Update(base.UpdateCommand):
  ...
  def Run(self, args):
    ...
    eg = args.CONCEPTS.intercept_endpoint_group.Parse()
    try:
      original = client.DescribeEndpointGroup(eg.RelativeName())
    except apitools_exceptions.HttpNotFoundError:
      raise exceptions.InvalidArgumentException(
          'intercept-endpoint-group',
          'Intercept endpoint group does not exist.',
      )
  ...
  def _process_label_updates(self, client, args, original_eg):
    ...
    labels_update = labels_diff.Apply(
        client.messages.InterceptEndpointGroup.LabelsValue,
        labels,
    )
  ...
  def _wait_for_operation(self, client, operation, eg, max_wait):
    return client.WaitForOperation(
        ...
        message=(
            'waiting for intercept endpoint group [{}] to be updated'.format(
                eg.RelativeName()
            )
        ),
        has_result=False,
